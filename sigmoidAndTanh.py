...
def tanh_derivative(x):
  return(1-x**2)
def NeuralNetwork_Tanh(X,Y,X_test,Y_test,weights1,weights2,weights3):
  testnumber = len(X)
  ...
  while(cnt<=100000):
    for aditya in range(testnumber):
      ...
      cnt+=1
      input1 = X[aditya]
      layer2 = np.array(tanh(dot(input1, weights1)))
      layer3 = np.array(tanh(dot(layer2, weights2)))
      output = np.array(tanh(dot(layer3, weights3)))
      del4 = np.array((Y[aditya] - output)*tanh_derivative(output))  
      del3 = np.array(dot(weights3, del4.T)*(tanh_derivative(layer3).T))
      del2 = np.array(dot(weights2, del3)*(tanh_derivative(layer2).T))
      layer3=np.array([layer3]).reshape(1,50)
      del4=np.array([del4]).reshape(1,1)
      ...
      weights1 += dot(input1.T, del2.T)*.1 
      weights2 += dot(layer2.T, del3.T)*.1
      weights3 += dot(layer3.T, del4)*.1
      layer2 = tanh(dot(X, weights1))
      layer3 = tanh(dot(layer2, weights2))
      output = tanh(dot(layer3, weights3))
      for i in range(len(output)):
        if(output[i]<.4):
          output[i]=0
        else:
          output[i]=1
      lol = (np.sum((abs(output-Y)**2)))
      a.append(lol)
      layer2 = tanh(dot(X_test, weights1))
      layer3 = tanh(dot(layer2, weights2))
      output = tanh(dot(layer3, weights3))
      for i in range(len(output)):
        if(output[i]<.4):
          ...
          output[i]=1
      lol = (np.sum((abs(output-Y_test)**2)))
      b.append(lol)
  insampleerror = min(a)
  ...
  plt.title("In Sample Error vs Iterations(Tanh)")
  plt.show()
  plt.plot(itr,b)
  plt.xlabel("Iterations")
  plt.ylabel("Number of Incorrect predictions")
  plt.title("Out Sample Error vs Iterations(Tanh)")
  plt.show()
  plt.plot(itr,a,'r',label="Insample Error")
  plt.plot(itr,b,'b',label="Outsample Error")
  plt.title("Insample && Outsample vs Iterations(Tanh)")
  plt.xlabel("Iterations")
  plt.ylabel("Error")
  plt.show()
...
def NeuralNetwork_Sigmoid(X,Y,X_test,Y_test,weights1,weights2,weights3):
  ...
  while(cnt<=100000):
    for aditya in range(testnumber):
      ...
      output = np.array(sigmoid(dot(layer3, weights3)))
      del4 = np.array((Y[aditya] - output)*sigmoid_derivative(output))  
      ...
      output = sigmoid(dot(layer3, weights3))
      for i in range(len(output)):
        if(output[i]<.4):
          output[i]=0
        else:
  ...
  plt.xlabel("Iterations")
  plt.ylabel("Number of Incorrect predictions")
  plt.title("In Sample Error vs Iterations(Sigmoid)")
  plt.show()
  plt.plot(itr,b)
  plt.xlabel("Iterations")
  plt.ylabel("Number of Incorrect predictions")
  plt.title("Out Sample Error vs Iterations(Sigmoid)")
  plt.show()
  plt.plot(itr,a,'r',label="Insample Error")
  plt.plot(itr,b,'b',label="Outsample Error")
  plt.title("Insample && Outsample vs Iterations(Sigmoid)")
  plt.xlabel("Iterations")
  plt.ylabel("Error")
  plt.show()
...
weights3 = np.array(2 * random.random((l3, 1)) -1)
print("***********************************************************")
...
print("***********************************************************")
print("TANH FUNCTION")
weights1 = np.array(2.0 * random.random((2000, l2)) -1)
weights2 = np.array(2.0 * random.random((l2, l3)) -1)
weights3 = np.array(2.0 * random.random((l3, 1)) -1)
NeuralNetwork_Tanh(X,Y,X_test,Y_test,weights1,weights2,weights3)
print("***********************************************************")
